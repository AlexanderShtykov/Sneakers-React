                ...
                <div className='items'>
                    {cartSneakers.map(sneakers => (
                        <div className='cartItem d-flex align-center mb-20'>
                            <div className='cartItemImg' style={{ backgroundImage: `url(${sneakers.imageUrl})`}}></div>
                            <div className='mr-20 flex'>
                                <p className='mb-5'>{sneakers.title}</p>
                                <b>{sneakers.price}</b>
                            </div>
                            <img onClick={() => onRemove(sneakers.id)} className='removeBtn' src='/img/btn-remove.svg' alt='Remove'></img>
                        </div>
                    ))}
                </div>
